        ...
        VStack(alignment: .leading, spacing: PPSpacing.md) {
            HStack {
                ...
                Button(role: .destructive, action: onRemove) {
                    Image(systemName: "xmark.circle.fill").foregroundColor(.ppTextTertiary(colorScheme))
                }
            }
            ...
            TextField("Account name", text: $account.name)
                ...
                .background(Color.ppSurface(colorScheme)).clipShape(RoundedRectangle(cornerRadius: PPRadius.sm))
                .overlay(RoundedRectangle(cornerRadius: PPRadius.sm).stroke(Color.ppBorder(colorScheme), lineWidth: 1))

            HStack {
                Text("Type").font(.ppCallout).foregroundColor(.ppTextSecondary(colorScheme))
                Spacer()
            ...
            HStack {
                Text("Starting Balance").font(.ppCallout).foregroundColor(.ppTextSecondary(colorScheme))
                Spacer()
            ...
            if account.showSpendLimit {
                HStack {
                    Text("Spend Limit").font(.ppCallout).foregroundColor(.ppTextSecondary(colorScheme))
                    Text("(optional)").font(.ppCaption).foregroundColor(.ppTextTertiary(colorScheme))
                    Spacer()
